...
- `defineMetadata`: Allows the user to define some metadata to be associated with this property. This metadata can then be accessed on the class via `Object.getMetadata`. See the section on Metadata below for more details.
...
Two of these objects may exist, one for static elements and the class itself, and one for non-static elements. These objects are then accessed using a new `Object.getMetadata` API, like so:

```js
Object.getMetadata(target, key);
```
...
Object.getMetadata(C.prototype, MY_META);
...
Object.getMetadata(C.prototype, VALIDATIONS).public.foo.length; // 2
```
...
Object.getMetadata(C.prototype, VALIDATIONS).public.m; // [1, 2]
```

Finally, the metadata object itself inherits from the parent class' metadata object if it exists, the `public` object inherits from the parent's `public` object if it exists, and the `private` array is a duplicate of the parent's `private` array (if it exists) with child values appended to the end:
...
This metadata API is semi-private, since users have no way of enumerating all of the keys which metadata has been defined under, and keys are required to be symbols which can be kept in a single module and never exported. Alternatively, users can take extra measures to ensure that their metadata is even more private. They can do this by exposing a _key_ in the metadata, rather than the metadata itself. This key can then be used to read the metadata from a private data store, only available in module scope for instance. For example, you could do this with an object and a WeakMap like so:

```js
HIDDEN_META = new WeakMap();
...
HIDDEN_META.get(Object.getMetadata(C.prototype).public.x);
// { secret: "values" }
```
...
class Container {
  ...
  create(Class) {
    ...
    const { public } = Object.getMetadata(instance);
...
class Container {
  ...
  create(Class) {
    ...
    const { public, private } = Object.getMetadata(instance);
...
class C {
  ...
  updateX() {
    let { get, set } = Object.getMetadata(this).private[0];
